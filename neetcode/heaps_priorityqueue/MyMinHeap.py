...
class MyMinHeap(object):
    ...
    @classmethod
    def buildMinHeapFromList(cls, inpList:list):
        mmh = cls()
        for item in inpList:
            mmh.insert( item )
        return mmh
    ...
    def decreaseKey(self, index:int, newValue:int) -> None:
        #It is believed i.e on user that the new value is less than the current value at the index.
        if newValue > self.data[index]:
            raise Exception( f"New value={newValue} should be less than the current value={self.data[index]} at the index={index}, as this function expects this. Raising exception because of wrong ussage" ) 
